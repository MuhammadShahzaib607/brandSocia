...
export default function ContactCTA() {
  ...
  useEffect(() => {
    ...
    // Immediately check if element is already in the viewport
    const rect = el.getBoundingClientRect();
    if (rect.top < window.innerHeight && rect.bottom > 0) {
      setIsVisible(true);
      // No observer needed if already visible
      return;
    }
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (entry.isIntersecting) {
          setIsVisible(true);
          // once visible, we can disconnect observer to avoid extra triggers
          observer.disconnect();
        }
      },
      { threshold: 0.1 }
